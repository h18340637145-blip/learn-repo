...
test("AI 应用开发 阶段 00-10 全部发布，覆盖 RAG 到端到端平台完整路线", () => {
  const aiApplication = getCourse("ai-application");

  assert.equal(aiApplication.stages.length, 11);
  assert.equal(aiApplication.stages.every((stage) => stage.lessons.every((lesson) => lesson.status === "published")), true);
  assert.equal(aiApplication.stages.every((stage) => stage.project.status === "published"), true);
  assert.equal(getLessonsByCourse("ai-application").length, 99, "AI 应用 应有 4 蓝图 + 7 真实阶段共 99 个可玩案例");
});

test("服务端工程 阶段 00-10 全部发布，覆盖微服务到平台工程完整路线", () => {
  const serverEngineering = getCourse("server-engineering");

  assert.equal(serverEngineering.stages.length, 11);
  assert.equal(serverEngineering.stages.every((stage) => stage.lessons.every((lesson) => lesson.status === "published")), true);
  assert.equal(serverEngineering.stages.every((stage) => stage.project.status === "published"), true);
  assert.equal(getLessonsByCourse("server-engineering").length, 99, "服务端工程 应有 4 蓝图 + 7 真实阶段共 99 个可玩案例");
});
