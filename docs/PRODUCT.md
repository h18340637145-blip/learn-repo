...
主要学习者包括 JavaScript 基础较弱、刚接触 Node.js 的新手，也包括已经掌握 JavaScript 基础语法、希望系统理解 Node.js 真实程序行为的学习者。课程从基础训练营开始，逐步进入模块、包、异步队列、文件、流、HTTP、进程、实时通信、测试、安全和生产诊断。
...
已发布内容的知识边界来自 Node.js 官方 Learn/API 文档与 JavaScript 官方参考资料，覆盖基础语法、Getting Started、Command Line、Modules、Packages、TypeScript、Asynchronous Work、Events、Timers、HTTP、Process、Worker Threads、Test Runner、Security 和 Diagnostics 等主题。
...
每个阶段包含 8 个知识点和 1 个阶段项目。阶段 00–03、05–10 已形成完整学习闭环，阶段 04 当前发布 2 个案例；下一轮课程工作应优先补齐阶段 04 剩余文件与 Stream 内容，并为已发布课程扩展更多题型和可视化类型。
