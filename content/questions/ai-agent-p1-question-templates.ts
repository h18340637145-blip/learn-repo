...
type AiAgentQuestionType = Extract<QuestionType, "diagnosis" | "repair" | "completion" | "execution-order">;

export type AiAgentQuestionBankSeedLesson = Pick<
  LessonSpec,
  "id" | "stageId" | "kind" | "title" | "concept" | "points" | "memoryHook" | "files" | "entryFile"
>;

type AiAgentQuestionProfile = Partial<Record<StageId, readonly AiAgentQuestionType[]>>;

type AiAgentBankBuilderOptions = {
  runtimeLabel: string;
  stageProfile: AiAgentQuestionProfile;
};

const COURSE_LABEL = "AI Agent" as const;

export function buildAiAgentP1QuestionBank(
  lessons: readonly AiAgentQuestionBankSeedLesson[],
  options: AiAgentBankBuilderOptions
): QuestionBankEntry[] {
...
function pickQuestionType(
  lesson: AiAgentQuestionBankSeedLesson,
  stageProfile: AiAgentQuestionProfile,
...
function createDiagnosisQuestion(
  ...
  return {
    id,
    type: "diagnosis",
    prompt: `阅读「${lesson.title}」的 Agent 运行现象，最可能的根因是什么？`,
    ...
    options: [
      {
        id: "a",
        label: `先确认 ${COURSE_LABEL} 循环 (Observe/Plan/Act/Reflect) 的运行边界`,
      ...
      },
      {
        ...
        detail: "忽略了 Agent 上下游的工具与记忆链路",
        feedback: "换模型掩盖不了 Prompt/工具/记忆链路问题，先看结构化 trace 与工具返回。"
      },
      {
        id: "c",
    ...
    answerId: "a",
    correctExplanation: `「${lesson.title}」的关键是：${lesson.memoryHook}。诊断时先确认 Agent 上下文、工具契约和 trace 源头，再改代码。`
  };
}
...
function createRepairQuestion(
  ...
  return {
    id,
    type: "repair",
    prompt: `如果「${lesson.title}」的示例在真实 Agent 服务里不稳定，哪种修复最符合当前知识点？`,
    ...
    options: [
      {
        id: "a",
        label: "保留 Agent 运行边界，并显式处理关键输入 / 工具返回",
      ...
      },
      {
        ...
        feedback: "裸 catch 或吞异常会让学习者看不到真实失败路径，生产 Agent 会加剧漂移与幻觉。",
        language: "ts",
        diffLines: [2, 4],
      ...
      },
      {
        id: "c",
        label: "换个 LLM 模型再跑一次",
        detail: "换模型不等于修好 Agent",
        feedback: `换模型不能替代对 ${COURSE_LABEL} 循环、工具与记忆的理解。`,
        language: "ts",
        diffLines: [1],
    ...
    answerId: "a",
    correctExplanation: `稳定修复要保留当前 Agent 运行模型，并把「${lesson.memoryHook}」对应的工具契约、记忆与 trace 处理清楚。`
  };
}
...
function createCompletionQuestion(
  ...
  return {
    id,
    type: "completion",
    prompt: `补全「${lesson.title}」的学习检查点，哪段代码最能表达这个 Agent 知识点？`,
    ...
    options: [
      ...
      },
      {
        ...
        label: "只声明数组却不打印",
        detail: "无法看到运行结果",
        feedback: "Agent 学习题需要可观察反馈，只定义数据不能验证心智模型。",
        language: "ts",
        diffLines: [3],
      ...
      },
      {
        ...
        detail: "看不到具体知识点",
        feedback: "固定输出无法说明当前 Agent 阶段的核心边界，也不利于迁移到真实系统。",
        language: "ts",
        diffLines: [4],
...
function createExecutionOrderQuestion(
  ...
  const entryFile = lesson.entryFile ?? lesson.files?.[0]?.name ?? "agent.ts";
  const orderItems = [
    `读取入口 ${entryFile}`,
    `应用知识点：${lesson.points[0] ?? lesson.title}`,
    "产生 trace / 工具返回 / Agent 输出"
  ];

  return {
    id,
    type: "execution-order",
    prompt: `学习「${lesson.title}」时，哪条 Agent 执行链路最可靠？`,
    ...
    options: [
      {
        ...
        feedback: "正确：这是可复用的 Agent 调试和学习顺序。"
      },
      {
        id: "b",
        label: "先看 LLM 最终回答 -> 猜原因 -> 再回头找入口",
        detail: "倒着排查容易误判 Agent 循环",
        feedback: "LLM 输出只是结果，不是入口；先找 Agent loop、工具调用与 trace root 能减少猜测。"
      },
      {
        id: "c",
        label: "跳过代码，只记结论",
        detail: "无法迁移到真实 Agent 系统",
        feedback: `只背结论不能解释 ${COURSE_LABEL} 在生产 Agent 里的执行路径。`
      }
    ],
    answerId: "a",
...
function getEntryCode(lesson: AiAgentQuestionBankSeedLesson): string {
  const code = lesson.files?.find((file) => file.name === lesson.entryFile)?.code ?? lesson.files?.[0]?.code ?? "";
  return code.split("\n").slice(0, 18).join("\n");
}

function createRepairCode(lesson: AiAgentQuestionBankSeedLesson, correct: boolean): string {
  const title = escapeForCode(lesson.title);
  const hook = escapeForCode(lesson.memoryHook);

  if (correct) {
    return `const topic = "${title}";\nconst model = "${hook}";\nif (!model) {\n  throw new Error("missing agent runtime model");\n}\nconsole.log(topic, model);`;
  }

  return `const topic = "${title}";\ntry {\n  console.log(topic);\n} catch (_err) {\n  console.log("ok");\n}`;
}

function createCompletionCode(lesson: AiAgentQuestionBankSeedLesson, complete: boolean): string {
...
function createIncompleteCode(lesson: AiAgentQuestionBankSeedLesson): string {
